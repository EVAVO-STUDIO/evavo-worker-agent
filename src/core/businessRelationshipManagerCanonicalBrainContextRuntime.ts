...
export const BUSINESS_RELATIONSHIP_MANAGER_CANONICAL_BRAIN_CONTEXT_RUNTIME_CONTRACT =
  "business_relationship_manager_canonical_brain_context_runtime_v2" as const;
...
function withoutMemoryDomain(values: readonly RelationshipSourceReadinessItem[] | null | undefined): readonly RelationshipSourceReadinessItem[] {
  const input = values ?? [];
  if (input.some((item) => item.domain === "memory")) throw new Error("RELATIONSHIP_MANAGER_CANONICAL_BRAIN_CALLER_MEMORY_READINESS_FORBIDDEN");
...
function memoryEvidenceItem(read: BrainMemoryContextReadResult): Relationship360EvidenceItem {
  ...
  const sourceRefs = Object.freeze([...new Set([
    read.stateEvidenceRef,
    ...records.flatMap((record) => record.sourceRefs),
  ])]);
  return Object.freeze({
    id: `brain-memory-state-${read.stateEvidenceRef.slice(-24)}`,
    domain: "memory",
...
function successfulMemoryReadiness(read: BrainMemoryContextReadResult): RelationshipSourceReadinessItem {
  if (read.context.records.length) return Object.freeze({
    domain: "memory", state: "verified", required: true,
    observedAt: read.context.generatedAt,
    sourceRefs: Object.freeze([read.stateEvidenceRef, ...read.context.records.flatMap((record) => record.sourceRefs)]),
    detail: `Brain returned ${read.context.records.length} durable relationship memory record${read.context.records.length === 1 ? "" : "s"}.`,
  });
  return Object.freeze({
    domain: "memory", state: "not_found", required: true, absenceAcceptable: true,
    observedAt: read.context.generatedAt, sourceRefs: Object.freeze([read.stateEvidenceRef]),
...
export async function runCanonicalRelationshipManagerCycleWithBrainContext(
  ...
  if (input.brain.contract !== "business_brain_memory_context_port_v2") {
  ...
  const memoryRequest = buildBusinessMemoryContextRequest({
    intent: "communication", relationshipId, personId,
    ...
    maximumRecords: 40, maximumCharacters: 30_000,
  ...
  try {
    ...
    if (read.context.asOf !== new Date(input.cycle.decisionAt).toISOString()) throw new Error("RELATIONSHIP_MANAGER_CANONICAL_BRAIN_AS_OF_MISMATCH");
    ...
    memorySourceReadiness = Object.freeze({
      domain: "memory", state: "provider_unavailable", required: true,
  ...
  const context: CanonicalRelationshipContextInput = Object.freeze({
    ...input.context,
    evidenceItems: Object.freeze([...input.context.evidenceItems, ...(memoryEvidence ? [memoryEvidence] : [])]),
  ...
  const canonicalCycle = runCanonicalRelationshipManagerCommunicationCycle({ cycle: input.cycle, context });
  if (brainState === "provider_unavailable" && canonicalCycle.approvalGradeReady) throw new Error("RELATIONSHIP_MANAGER_CANONICAL_BRAIN_UNAVAILABLE_READINESS_WIDENED");
  if (read && !canonicalCycle.decisionContext.evidenceRefs.includes(read.stateEvidenceRef)) {
    throw new Error("RELATIONSHIP_MANAGER_CANONICAL_BRAIN_STATE_EVIDENCE_NOT_BOUND");
  }

  return Object.freeze({
    contract: BUSINESS_RELATIONSHIP_MANAGER_CANONICAL_BRAIN_CONTEXT_RUNTIME_CONTRACT,
    brainState,
    stateEvidenceRef: read?.stateEvidenceRef ?? null,
