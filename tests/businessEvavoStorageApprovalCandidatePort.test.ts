...
const authorityId = "storage-authority-1";
...
function successFetch(observed: { input?: string; init?: RequestInit }, instanceId = authorityId): ApprovalCandidateFetch {
      ...
      async json() {
        return {
          ...
          result: {
            ...
            storageAuthority: { system: "evavo-storage", instanceId },
            storage: {
...
test("persists exact candidate through governed EVAVO Storage action", async () => {
  ...
  const port = createEvavoStorageApprovalCandidatePort(config(), successFetch(observed));

  const result = await port.persist(value);
  assert.equal(port.contract, "business_evavo_storage_approval_candidate_port_v2");
...
test("refuses missing expected Storage authority before any write", () => {
  assert.throws(() => createEvavoStorageApprovalCandidatePort({
    ...config(),
    expectedAuthorityId: " ",
...
test("rejects a successful write receipt from the wrong Storage authority", async () => {
  const port = createEvavoStorageApprovalCandidatePort(config(), successFetch({}, "storage-authority-other"));
  await assert.rejects(() => port.persist(candidate()), /AUTHORITY_MISMATCH/);
});

test("does not surface remote error message payloads", async () => {
  const port = createEvavoStorageApprovalCandidatePort(config(), async () => ({
    ok: false,
    status: 403,
    async json() {
      return {
...
test("rejects successful response that lacks native immutable Storage binding", async () => {
  const port = createEvavoStorageApprovalCandidatePort(config(), async (_input, init) => {
      ...
      async json() {
        return {
          ...
          result: {
            ...
            recordedAt: "2026-09-04T08:00:01.000Z",
            storageAuthority: { system: "evavo-storage", instanceId: authorityId },
          },
        };
      },
    };
