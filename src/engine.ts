...
function shouldRetrySourceLead(lead: LeadRow): boolean {
  ...
  const profilesVisited = Number(signals?.profilesVisited || 0);
  const extracted = Number(signals?.extractedCandidates || 0);
  return status === "failed" || (status === "do_not_contact" && profilesVisited > 0 && extracted === 0);
}
    ...
    try {
      ...
      if (looksLikeSourcePage(lead.website_url, html)) {
        ...
        if (expansion.newLeadIds.length > 0) {
          const fresh = await listLeads(env, { status: "new", limit: maxItems * 6 });
