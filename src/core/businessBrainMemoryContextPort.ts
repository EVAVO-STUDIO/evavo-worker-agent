...
export const BUSINESS_BRAIN_MEMORY_CONTEXT_PORT_CONTRACT =
  "business_brain_memory_context_port_v2" as const;
...
type BrainToolCallResponse = Readonly<{
  name?: unknown; ok?: unknown; output?: unknown; approvalRequired?: unknown;
}>;
...
function baseUrl(value: string): string {
  ...
  let parsed: URL;
  try { parsed = new URL(clean); } catch { throw new Error("BRAIN_MEMORY_CONTEXT_BASE_URL_INVALID"); }
...
function record(value: unknown, index: number): BrainMemoryContextRecord {
  ...
  const canonicalOwner = typeof raw.canonicalOwner === "string" && raw.canonicalOwner.trim() ? raw.canonicalOwner.trim() : undefined;
...
function parseOutput(value: unknown): BrainMemoryContextReadResult {
  ...
  const stateEvidenceRef = text(raw.stateEvidenceRef, "state_evidence_ref", 300);
  if (!/^brain:memory-context-state:[a-f0-9]{64}$/.test(stateEvidenceRef)) throw new Error("BRAIN_MEMORY_CONTEXT_STATE_EVIDENCE_INVALID");
...
function validateRequest(request: BrainMemoryContextRequest): void {
  ...
  if (!request.entityRefs.some((entity) => entity.kind === "relationship" && entity.id.trim())) throw new Error("BRAIN_MEMORY_CONTEXT_RELATIONSHIP_ENTITY_REQUIRED");
    ...
    async read(request: BrainMemoryContextRequest): Promise<BrainMemoryContextReadResult> {
      ...
      try {
        response = await fetchFn(endpoint, {
          ...
          body: JSON.stringify({ name: "brain_memory_context_v2", input: request, autonomy: "auto_low_risk" }),
      ...
      try { payload = await response.json() as BrainToolCallResponse; } catch { throw new Error("BRAIN_MEMORY_CONTEXT_RESPONSE_INVALID"); }
      ...
      if (payload.name !== "brain_memory_context_v2" || payload.ok !== true || payload.output === undefined) throw new Error("BRAIN_MEMORY_CONTEXT_TOOL_RESULT_INVALID");
